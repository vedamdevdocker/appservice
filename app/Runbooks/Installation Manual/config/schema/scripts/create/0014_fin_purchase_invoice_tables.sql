...
CREATE TABLE fin.purchaseinvoice (
...
-- Create purchaseinvoicelines table
CREATE TABLE fin.purchaseinvoicelines (
    ...
    created_by INT,
    updated_by INT,
    FOREIGN KEY (header_id) REFERENCES fin.purchaseinvoice(header_id),
    FOREIGN KEY (item_id) REFERENCES com.items (item_id),
    FOREIGN KEY (uom_id) REFERENCES com.uom (uom_id),
    UNIQUE (line_number, header_id)
) AUTO_INCREMENT = 1200;

-- Create purchaseinvoiceaccounts table
CREATE TABLE fin.purchaseinvoiceaccounts (
    line_id INT AUTO_INCREMENT PRIMARY KEY,
    line_number VARCHAR(50) NOT NULL,
    header_id INT NOT NULL,
