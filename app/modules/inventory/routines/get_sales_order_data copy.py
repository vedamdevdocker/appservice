...
def get_sales_order_data(sales_orders, sales_order_status, mydb, appuserid, MODULE_NAME):
    ...
    - sales_order_status (str): The status of the sales orders to be retrieved.
    ...
    if sales_orders:
        ...
        header_query = f"""
            ...
            WHERE so_num IN ({header_ids_placeholder}) AND status = %s
        """
        params = tuple(sales_orders) + (sales_order_status,)
        logger.debug(f"{appuserid} --> {MODULE_NAME}: header ids placeholder and params {header_ids_placeholder}: {params}")        
    else:
    ...
    line_query = f"""
        ...
        WHERE header_id IN ({header_ids_to_query}) AND status = %s
    """
    params = tuple(header_ids) + (sales_order_status,)
    ...
    logger.debug(f"{appuserid} --> {MODULE_NAME}: inpput information orders byheader  {orders_by_header}")    
