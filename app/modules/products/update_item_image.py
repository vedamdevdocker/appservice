...
update_item_images_api = Blueprint('update_item_images_api', __name__)

@update_item_images_api.route('/update_item_image', methods=['PUT'])
...
def update_item_image():
    ...
    # Retrieve item_id and image_id from request.form
    item_id = request.form.get('item_id')
    image_id = request.form.get('image_id')
    new_image_file = request.files.get('new_image_file')

    logger.debug(f"{appuser} --> {__name__}: Input Item id  --> {item_id}")  
    ...
    try:
        ...
        # Update the image in com.item_images table
        update_image_query = """
            UPDATE com.item_images 
        ...
        mydb1.commit()
        logger.info(f"{appuser} --> {__name__}: Image successfully updated for item_id={item_id}, image_id={image_id}")
