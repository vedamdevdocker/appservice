...
purchase_invoice_api = Blueprint('purchase_invoice_api', __name__)

@purchase_invoice_api.route('/create_purchase_invoice', methods=['POST'])
...
def create_purchase_invoice():
    try:
        ...
        # Log entry point
        logger.debug(f"{appuser} --> {__name__}: Entered the 'create_purchase_invoice' function")
        ...
        try:
            ...
            logger.info(f"{appuser} --> {__name__}: Purchase invoice data created successfully")
            mycursor.close()
            mydb.close()
            ...
            logger.error(f"{appuser} --> {__name__}: Unable to create purchase invoice data: {str(e)}")
            mycursor.close()
            mydb.close()
