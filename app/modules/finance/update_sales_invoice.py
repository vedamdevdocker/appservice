...
update_sales_invoice_api = Blueprint('update_sales_invoice_api', __name__)

@update_sales_invoice_api.route('/update_sales_invoice', methods=['PUT'])
...
def update_sales_invoice():
    ...
    try:
        ...
        logger.debug(f"{appuser} --> {__name__}: Entered the 'update_sales_invoice' function")
        ...
        try:
            ...
            if mycursor.rowcount > 0:
                # Log success
                logger.info(f"{appuser} --> {__name__}: Updated sales invoice")
                # Close the cursor and connection
                mycursor.close()
                mydb.close()
                # Return success message
                return jsonify({'status': True, 'message': 'Sales Invoice updated successfully'}), 200
            else:
            ...
            logger.error(f"{appuser} --> {__name__}: Unable to update sales invoice: {str(e)}")
            mycursor.close()
            mydb.close()
