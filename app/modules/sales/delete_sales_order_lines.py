...
delete_sales_order_lines_api = Blueprint('delete_sales_order_lines_api', __name__)

# Define the route
@delete_sales_order_lines_api.route('/delete_sales_order_lines', methods=['DELETE'])
...
def delete_sales_order_lines():
    try:
        ...
        header_exists = check_header_exists_in_lines(mydb, header_id)
        if not header_exists:
            return jsonify({'error': f"Header with ID {header_id} not found in lines table."}), 400
        ...
        if deleted_all_lines_of_header and not line_ids:
            response_message = f"All the Sales Order lines of the header: {header_id} are deleted successfully"
            success = True
            ...
            response_message = f"All the given {deleted_line_ids} sales order lines of the header: {header_id} are deleted successfully"
            success = True
        ...
        if "header_id is required" in error_message:
            ...
            if 'undeleted_line_ids' in locals() and undeleted_line_ids:
                return jsonify({'error': f"Some line_ids could not be deleted: {undeleted_line_ids}. Error: {error_message}"}), 500
            else:
                return jsonify({'error': f"An error occurred while deleting sales order lines. Error: {error_message}"}), 500
...
def delete_line_from_database(mydb, header_id, line_id):
    ...
    try:
        ...
        print("Entered delete line from database function")
...
def update_totalamount(mydb, header_id):
    try:
        ...
        # Update total amount in sal.sales_order_headers table
...
def check_header_exists_in_lines(mydb, header_id):
    try:
        # Query to check if header_id exists in lines table
        select_query = """
            SELECT COUNT(*) 
            FROM sal.sales_order_lines 
            WHERE header_id = %s
        ...
        # Check if any record exists with the given header_id
        return result[0] > 0
